...
export async function getSummary(): Promise<AssessmentSummary> {
  const result = await pool.query(
    `SELECT COUNT(*)::int AS total_sites, COUNT(*) FILTER (WHERE status='complete')::int AS complete_sites, COUNT(*) FILTER (WHERE status='draft')::int AS draft_sites, COALESCE(SUM(chicken_count),0)::int AS total_chickens, COUNT(*) FILTER (WHERE urgency='Immediate response')::int AS urgent_sites, COUNT(*) FILTER (WHERE condition='Good')::int AS good, COUNT(*) FILTER (WHERE condition='Moderate')::int AS moderate, COUNT(*) FILTER (WHERE condition='Bad')::int AS bad FROM assessments`,
  );
